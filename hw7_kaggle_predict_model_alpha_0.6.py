...
student_model_filename = './hw7_kaggle_predict_model_alpha_0.6.bin'

# region: Knowledge Distillation Loss
# Loss = alpha * T^2 * KL(Teacher's Logits / T || Student's Logits / T) + (1-alpha)(original Loss)
def loss_fn_kd(outputs, labels, teacher_outputs, T=20, alpha=0.6):
...
model_8b_filename  = './hw7_kaggle_predict_model_alpha_0.6_8_bit.bin'
...
student_net.eval()
prediction = []
predict_filename = 'hw7_kaggle_predict_SGD__alpha_0.6_liu.csv'
with torch.no_grad():
